...
let product = localStorage.setItem('allProducts',JSON.stringify(products))
let productRow = document.getElementById('products-row')
let str = '';

products.forEach((prod) => {
    str +=`<div class="col-12 col-md-6 col-lg-3 mb-4">
                <div class="card h-100 bg-black" style="width: 20rem;">
                    <img src="${prod.thumbnail}" class="card-img-top" alt="product-image">
                        <div class="card-body">
                            <h5 class="card-title text-white">${prod.title}</h5>
                                <p class="card-text text-white">$${prod.price}
                                  <span class="badge bg-success">${prod.discountPercentage}% OFF</span>
                                </p>
...
function addToCart(id) {
  ...
  let product = cart.find(p=> p.id == id);
    ...
    if(product){
      ...
      let prd = products.find(e=> e.id == id) 
      let string = {  
          "id": prd.id,    
